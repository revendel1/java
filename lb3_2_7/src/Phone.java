...
public class Phone {
    ...
    public LocalTime getTime_in_city() {
        return this.time_in_city;
    }
    public void setTime_in_city(LocalTime time_in_city) {
        this.time_in_city = time_in_city;
    }

    public LocalTime getTime_between() {
        return this.time_between;
    }
    public void setTime_between(LocalTime time_between) {
        this.time_between = time_between;
    }
    ...
    // Генерация сущностей
    public static Phone[] generatePhones(int count) {
        Phone[] phones = new Phone[count];
        String[] surnames = new String[]{"Иванов", "Петров", "Сидоров", "Козлов"};
        String[] names = new String[]{"Иван", "Петр", "Сидор", "Козел"};
        String[] patronymics = new String[]{"Иванович", "Петрович", "Сидорович", "Козлович"};
        String[] addresses = new String[]{"Москва", "Петербург", "Казань"};

        for (int i = 0; i < count; ++i) {
            String surname = surnames[(int) (Math.random() * (double) surnames.length)];
            String name = names[(int) (Math.random() * (double) names.length)];
            String patronymic = patronymics[(int) (Math.random() * (double) patronymics.length)];
            String address = addresses[(int) (Math.random() * (double) addresses.length)];
            String time_in_city = String.format("%02d",(int) (Math.random() * 24)) + ":" + String.format("%02d",(int) (Math.random() * 60)) + ":" + String.format("%02d",(int) (Math.random() * 60));
            String time_between = (int) (Math.random() * 2) > 0 ? String.format("%02d",(int) (Math.random() * 24)) + ":" + String.format("%02d",(int) (Math.random() * 60)) + ":" + String.format("%02d",(int) (Math.random() * 60)) : "00:00:00";

            phones[i] = new Phone(i+1, surname, name, patronymic, address, (i+1) * 100, (i+1) * 200, (i+1) * 300, LocalTime.parse(time_in_city), LocalTime.parse(time_between));
        }
